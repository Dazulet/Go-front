  ...
  return (
    ...
    >
      <div className="relative rounded-[4px] overflow-hidden border border-neon-blue/15">
        ...
        <div className="absolute right-0 top-0 bottom-0 w-1/2 bg-gradient-to-l from-neon-blue/5 to-transparent" />
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 px-6 sm:px-10 py-7">
          <div>
            ...
            <h3 className="text-xl sm:text-2xl font-display font-black text-text-primary">
              Unlock <span className="gradient-text">50,000+</span> Titles
            </h3>
            <p className="text-sm text-text-muted mt-1">Read without limits. New chapters every day.</p>
          </div>
          <div className="flex gap-3 flex-none">
            <a href="/register" className="px-5 py-2.5 bg-neon-blue text-void font-display font-bold text-sm rounded-sm hover:bg-blue-400 transition-colors shadow-neon-blue whitespace-nowrap">
              Start Free Trial
            </a>
            <a href="/catalog" className="px-5 py-2.5 bg-white/8 border border-white/12 text-text-secondary font-medium text-sm rounded-sm hover:bg-white/12 transition-colors whitespace-nowrap">
              Browse Free
            </a>
          </div>
        </div>
...
export default function HomePage() {
  const trending   = MOCK_MANGA.filter(m => m.isTrending)
  const popular    = MOCK_MANGA.filter(m => m.isPopular)
  const newTitles  = MOCK_MANGA.filter(m => m.isNew)
  const completed  = MOCK_MANGA.filter(m => m.status === 'completed')
  const allManga   = MOCK_MANGA
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="space-y-12 pb-20 pt-2">
        ...
        {/* Two-column: New Chapters + Top Ranking */}
        ...
        <MangaRow
          ...
          manga={completed}
          linkTo="/catalog?status=completed"
        />
